...
function solution(progresses, speeds) {
  ...
  for (let i = 0, j = 0; i < daysNeeded.length; i++) {
    //만약 daysNeeded 값이 maxDay의 값보다 작거나 같으면 이미 완성된 작업이기 때문에 answer의 값을 1로 설정해준다
    if (daysNeeded[i] <= maxDay) {
      answer[j] += 1;
    } else {
      //완성된 작업이 아니라면 maxDay에 daysNeeded의 값을 넣어주고
      maxDay = daysNeeded[i];
      //answer의 다음 요소에 1을 넣어준다
      answer[++j] = 1;
    }
  }
  return answer;
